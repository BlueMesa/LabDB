...
namespace Bluemesa\Bundle\CalendarBundle\Controller;
...
class CalendarController extends AbstractController
{
    ...
    public function calendarAction($username)
    {
        ...
        $om = $this->getObjectManager('Bluemesa\Bundle\FliesBundle\Entity\Vial');
        $calendar = new VCalendar();
        $calendar->VERSION = '2.0';
        $field = 'X-WR-CALNAME';
        $calendar->$field = $user->getShortName() . '\'s flywork';

        $stockDates =  $om->getRepository('Bluemesa\Bundle\FliesBundle\Entity\StockVial')->getFlipDates($user);
        ...
        $crossDates =  $om->getRepository('Bluemesa\Bundle\FliesBundle\Entity\CrossVial')->getFlipDates($user);
